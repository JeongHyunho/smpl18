...
| Use | read-only decoding of `.b3d` containers by `smpl18.formats.b3d` |
...
From the package root (`packages/smpl18`, or the repository root once the package is its own
repository), with a `protoc` whose `--version` prints `libprotoc 3.21.9`:

```powershell
protoc.exe `
  --proto_path=src/smpl18/formats/_vendor/nimblephysics `
  --python_out=src/smpl18/formats/_vendor/nimblephysics `
  src/smpl18/formats/_vendor/nimblephysics/SubjectOnDisk.proto
```
...
named above, not from the `.proto`, which describes only the messages. `smpl18.formats.b3d`
